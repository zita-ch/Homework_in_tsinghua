...
def run():
    ...
    def search():
        ...
        s = e.get()
        if s=="":
            tk.messagebox.showerror('error', 'searching for null is not allowed')
        aa=time.time()
        if s[0:5]=="bool(" and s[-1]==")":
            print("searching for:", s[5:-1])
            r=ini_and_func.bool_calculate(s[5:-1])
    ...
    def view_article():
        ...
        try:
            art_win = tk.Toplevel()
            art_win.title(ini_and_func.filelist[current_articles[rank_art]])
            ...
            def get_art():
                ...
                f = open(ini_and_func.path + "\\" + name, 'r')
                content = f.read()
                s = re.findall("<pre>\n\n\n([\S\s]*)</pre>", content)[0]
                return s
            ...
            c4 = tk.Button(art_win,
                           text='view the previous searched article',  # 显示按钮上的文字
                           command=refresh_art_pre)  # 点击按钮执行的命令
            c4.place(width=200, height=30, x=50, y=350)

            c5 = tk.Button(art_win,
                           text='view the next searched article',  # 显示按钮上的文字
                           command=refresh_art_next)  # 点击按钮执行的命令
            c5.place(width=200, height=30, x=350, y=350)

        except:
            tk.messagebox.showerror('error','no results for viewing')

    c3 = tk.Button(window,
                  text='view the searched articles',  # 显示按钮上的文字

...
for i in filelist:
    ...
    titles.append(gettitle(soup))
    content=soup.get_text().replace('\n',' ').replace('`',' ').strip()
    temp_words = [term.word for term in HanLP.segment(content) if term not in punctuations]
...
for i in range(num_docs):
    f = open(path + "\\" + filelist[i], 'r',encoding='utf-8')
    content = f.read()
    soup=BeautifulSoup(content)
    content=soup.get_text().replace('\n',' ').replace('`',' ').strip()
    temp_words = [term.word for term in HanLP.segment(content) if term not in punctuations]
...
wr2=open("inverted_index.csv",'w',encoding='utf-8')
skip_steps=[]
...
wr4=open("pos_index.csv",'w',encoding='utf-8')
for w in words:
...
for i in range(len(inv_index)):
    ...
    for j in range(len(inv_index[i])):
        ...
        print(lines)###########debug
##################建立和记录skippoints
wr3=open("skip_points.csv","w",encoding='utf-8')
for i in range(num_words):
    word=words[i]
    index=word_doc_dict[word]
...
wrb=open('bigram_index.csv','w',encoding='utf-8')
